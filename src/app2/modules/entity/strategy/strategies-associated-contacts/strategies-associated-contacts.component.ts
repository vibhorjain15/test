...
@Component({
  selector: 'strategies-associated-contacts',
  templateUrl: './strategies-associated-contacts.component.html',
  styleUrls: ['./strategies-associated-contacts.component.css'],
})
export class StrategiesAssociatedContactsComponent implements OnInit {
  strategy;
  related_contacts = [];
  public_contacts = [];
  isFirmOwner;
    ...
    this.user.pipe(take(1))
      .subscribe((data) => {
        if (data) {
          ...
          if (
            Number(this.stateParams.firmId) === this.current_user.firmInfo.id
          ) {
  ...
  getRelatedContacts() {
    this.StrategyDataservice.getRelatedContacts(this.strategyId).subscribe(
      (response: any) => {
  ...
  addContact() {
    if (!this.isFreeSubscription) {
      this.strategy.firm_id = this.strategy.parentFirm.id;
      this.customModalService.invoke('manage-contact', {
        initialState: {
          entity_details: this.strategy,
          entity_type: keywordConstants.Strategy,
  ...
  navigateToContacts(entityId) {
    this.routerService.navigateWithParams(`app.contacts`, { Id: entityId });
  }
