...
@Component({
  selector: 'my-firm-aum-tr',
  templateUrl: './my-firm-aum-tr.component.html',
  styleUrls: ['./my-firm-aum-tr.component.css'],
})
export class MyFirmAumTrComponent implements OnInit {
  firm;
  entity_type;
  tables = [];
  currentFirmId;
  ...
  aumActions = [
    {
      label: 'Add AUM/Track Record',
      key: 'add',
    },
    {
      label: 'Upload AUM/Track Record',
      key: 'upload',
    },
  ];

  @Output() onUnsavedTableCountChange = new EventEmitter();
    ...
    this.user.pipe(take(1))
      .subscribe((data) => {
        if (data) {
          ...
          this.firmDataService.getFirm(this.currentFirmId).subscribe((firm) => {
            this.firm = firm;
            this.getTables();
          });
        }
  ...
  onDropdownClick(aumAction: any) {
    if (aumAction.key === 'add') {
      this.openAddTableDialog();
    } else if (aumAction.key === 'upload') {
