...
@Component({
  selector: 'vehicle-ddqs',
  templateUrl: './vehicle-ddqs.component.html',
  styleUrls: ['./vehicle-ddqs.component.css'],
})
export class VehicleDdqsComponent implements OnInit, OnChanges {
  @Input() modalTitle = '';
  @Input() ddqType = '';
  @Input() readonly;
  ...
  redirectToQuestionnaire(ddq) {
    ...
    if (
      ddq.entity_type.toLowerCase() === keywordConstants.Vehicle.toLowerCase()
    ) {
      //if diligence type is vehicle then go to firms.vehicles route
      newState = '.firms.strategies';
      newParams.fromfirmId = ddq.fromfirm_id;
      newParams.tofirmId = ddq.tofirm_id;
      newParams.vehicleId = ddq.entity_id;
    } else if (
