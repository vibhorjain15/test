export default function getDocDefinition(
  ...
  function getRowsToExport(columnsToExport) {
    ...
    agGridApi.forEachNodeAfterFilterAndSort((node) => {
      ...
      let rowToExport = columnsToExport.map(({ colId, colDef }) => {
        let cellValue;
        if (colId === "document_types") {
          cellValue = node.data?.document_types?.join(", ");
        } else if (colId === "contacts") {
          const item = node.data.contacts.map((x) => x.name);
          cellValue = item.join(", ");
        } else if (
          colDef?.cellRendererParams?.type == "rating" &&
          node.data?.[colId]?.field_value?.length > 0
        ) {
          return node.data?.[colId]?.display_format == "name"
            ? node.data?.[colId].field_value[1]
            : node.data?.[colId].field_value[0];
        } else {
          cellValue = agGridApi.getValue(colId, node);
        }
