  ...
  var open = function (editor) {
    ...
    var selectIndex = (function () {
      if (selectedNode.className == "fnoteWrap") {
        var num = selectedNode.childNodes[0].firstChild.nodeValue.replace(
          /[^0-9]/g,
          ""
        );
        return num;
    ...
    if (isFootNotes) {
      name =
        selectedNode.name ||
        decodeURIComponent(
          selectedNode.childNodes[0].getAttribute("data-content")
        ) ||
        "";
    }
    ...
    editor.windowManager.open({
      ...
      onSubmit: function (e) {
        var newfootnoteContent = e.getData().name,
          ...
          htmlTemplate =
            '<span class="fnoteWrap" id="#wk_ft{FOOTNOTE_INDEX}" contenteditable="false"><sup class="fnoteBtn" title="' +
            newfootnoteContent +
            '" data-content="' +
            fixFootnoteContent +
            '">{FOOTNOTE_INDEX}</sup></span>&nbsp;',
          ...
          var getNext = function (el) {
            ...
            nextAll = [].filter.call(
              ...
              function (htmlElement) {
                return htmlElement.previousElementSibling === el
                  ? (nextAll = true)
                  : nextAll;
              }
            );
        ...
        if (nextFD.length) {
          ...
          if (selectIndex < totalCount) {
            ...
            // anywhere add
            html = replaceTmpl(htmlTemplate, {
              FOOTNOTE_INDEX: $(totalFootNote[foundIdx]).html(),
            });
        ...
        var fnoteBtn = Array.from(
          editor.getDoc().querySelectorAll(".fnoteBtn")
        );
  ...
  function Plugin() {
    global.add("footnotes", function (editor) {
      editor.ui.registry.addIcon(
        ...
        '<img src="' +
          tinyMCE.baseURL +
          "/plugins/footnotes/img/fn.png" +
          '" aria-label="footnote" title="footnote" role="button">'
      );
