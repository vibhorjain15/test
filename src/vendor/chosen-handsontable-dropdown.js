...
var MultiSelectEditor = /*#__PURE__*/function (_TextEditor) {
  ...
  _createClass(MultiSelectEditor, [{
    ...
    value: function prepare(row, col, prop, td, originalValue, cellProperties) {
      _get(_getPrototypeOf(MultiSelectEditor.prototype), "prepare", this).call(this, row, col, prop, td, originalValue, cellProperties);
    ...
    value: function open() {
      ...
      var getOptions = function getOptions() {
        ...
        return Promise.resolve(toResolve).then(function (availableOptions) {
          ...
          if (_ramda["default"].isEmpty(originalValue) || _ramda["default"].isNil(originalValue)) {
            return availableOptions;
          }

          var selectedValues = originalValue.split(_this.separator);
          return _ramda["default"].map(function (item) {
            var label = "".concat(_ramda["default"].prop(_this.labelKey, item));
            return _ramda["default"].any(_ramda["default"].identical(label), selectedValues) ? _ramda["default"].assoc('selected', true, item) : item;
          }, availableOptions);
        });
      };
    ...
    */
    ...
    value: function showEditableElement() {
      ...
      this.textareaParentStyle.width = "".concat(this.getEditedCell().getBoundingClientRect().width, "px");
    ...
    value: function onBeforeKeyDown(event) {
      ...
      switch (event.keyCode) {
        case keyCodes.ARROW_RIGHT:
          if (_handsontable["default"].dom.getCaretPosition(target) !== target.value.length) {
            event.stopImmediatePropagation();
          }
