/*
 Implementation: src/app/shared/factories/onboarding.factory.coffee
 Library: https://github.com/nozzle/onboard.js/tree/master
 Use: To provide a product / app tour
 Modifications done to the lib: Added custom handlers like `onPrevious`, `onNext` etc.
*/
(function($) {
    ...
    var onboard = function() {
        var container,
            ...
            skipButton = $("<button type='button'/>").addClass("btn btn-default").html("Skip"),
            ...
            getElement = function(i, start, callback) {
                ...
                if (!element.length || element.width() <= 0 && element.height() <= 0) {
                    if (count) {
                        ...
                        loaderMessage.html('Something went wrong with this tour, pls try again!');
                        setTimeout(function() {
                            clearGuide();
                            if (afterExit) {
                                afterExit();
            ...
            gotoStep = function(i) {
                ...
                if (typeof steps[i].options != 'undefined') {
                    if (typeof steps[i].options.beforeEntering != 'undefined') {
                        steps[i].options.beforeEntering();
                    }
                    if (typeof steps[i].options.onSkip != 'undefined') {
                        onSkipStep = steps[i].options.onSkip;
                    }
                }
                ...
                if (steps) {
                    if (position > -1) {
                        if (typeof steps[position].options != 'undefined') {
                            if (typeof steps[position].options.onNext != 'undefined') {
                                steps[position].options.onNext();
                            }
                        }
                    }
                    position++;
            ...
            prevStep = function() {
                if (steps) {
                    if (position > -1) {
                        if (typeof steps[position].options != 'undefined') {
                            if (typeof steps[position].options.onPrevious != 'undefined') {
                                steps[position].options.onPrevious();
                            }
                        }
                    }
                    position--;
            ...
            positionBubble = function(i) {
                ...
                $(".circled-initials", bubble).html(i + 1);
                if (steps) {
                ...
                getElement(i, true, function(element) {
                    ...
                    if (width > $(window).height()) {
                        ...
                        if ((left + width + bubble.outerWidth()) > bodyContainer.width()) {
                    ...
                    if (position == (steps.length - 1)) {
                        ...
                        skipButton.addClass("hide");
                    } else {
                        nextButton.html("Next").removeClass("btn-danger");
                        skipButton.removeClass("hide");
                    }
        ...
        return {
            init: function(opts) {
                ...
                if (typeof opts != 'undefined') {
                    ...
                    if (typeof opts.onSkip != 'undefined') {
                        onSkip = opts.onSkip;
                    }
                }
                ...
                bubble.css("z-index", zIndex + 2).html("").append(arrow).append($("<div/>").addClass("circled-initials pos-abs clear-margin").html("1")).append($("<div/>").addClass("onboard")).append($("<div/>").addClass("pull-left").append(skipButton)).append($("<div/>").addClass("btn-group pull-right").append(prevButton).append(nextButton));
