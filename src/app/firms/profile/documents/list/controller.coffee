class FirmsDocumentsListController extends BaseController

  @register 'FirmsDocumentsListController'

  @inject 'FirmDataservice', '$stateParams', '$scope', '$state', 'DocumentListResource',
          'Restangular', 'toaster', 'SweetAlert', 'Utils', 'ModalFactory', 'DocumentsService', '$rootScope', 'DocumentDataservice', 'entityTypeValue', 'angularEnabled'

  initialize: ->
    @is_investor = @Utils.isInvestor()
    @entity_id = @$stateParams.firmId
    ...
    @documents = @DocumentListResource.$new({entity_id: @entity_id, entity_type: 'Firm', q: @searchText})
    @searchResultTemplateUrl = 'shared/ui-grid-cell-templates/search-result-popover.html'

    # @$rootScope.$on 'modal.closed', (ev) =>
    #   @documents.refresh()

    @$scope.$parent.FirmsDocumentsListController = @
  ...
  generatePageUrl: (entity)=>
    pageUrl = "app/firms/#{entity.entity_id}/profile/documents/#{entity.id}"
    pageUrl
