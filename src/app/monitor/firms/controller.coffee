class MonitorFirmsController extends BaseController

  @register 'MonitorFirmsController'

  @inject 'FirmsResource', 'Restangular', '$http', '$scope', 'FundDataservice', '$timeout', 'DueDiligenceDataservice', 'ModalFactory', 'Utils','$state','total_entity_records','CommonService', 'FILTER_TERNARY_OPERATORS', 'keywordConstants','angularEnabled'
  ...
  openRow: (row,col)=>
    if !row.internalRow && col.field != "selectionRowHeaderCol"
      @$state.go("app.firms.profile.monitor",{firmId: row.entity.id})
  ...
  loadCustomFields: ->
    @Restangular.all('service/dvapi_service/get_custom_fields').post({schema_type : "firm"}).then (response) =>
      @dynamicGridColumns = response.custom_fields.firm
      @firms = @FirmsResource.$new({
        options:{ include_contacts: false, is_active : true, include_custom_fields: true, include_dates: true,filters: {} },
        dynamic_grids_options: @dynamicGridColumns
      })
