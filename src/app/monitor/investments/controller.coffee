class MonitorInvestmentsController extends BaseController

  @register 'MonitorInvestmentsController'

  @inject 'InvestmentsResource', 'Restangular', '$scope', '$http', 'FundDataservice', '$timeout', 'DueDiligenceDataservice', 'ModalFactory', 'Utils', '$state', 'total_entity_records', 'CommonService', 'FILTER_TERNARY_OPERATORS', 'keywordConstants','angularEnabled'
  ...
  openRow: (row,col)=>
    if !row.internalRow && col.field != "selectionRowHeaderCol"
      @$state.go("app.firms.funds.profile.monitor",{firmId: row.entity.firm_id, fundId: row.entity.id})
  ...
  loadCustomFields: ->
    @Restangular.all('service/dvapi_service/get_custom_fields').post({schema_type : "fund"}).then (response) =>
      @dynamicGridColumns = response.custom_fields.fund
      @investments = @InvestmentsResource.$new({
        options:{ include_contacts: false, include_custom_fields: true, include_dates: true,filters: {} },
        dynamic_grids_options: @dynamicGridColumns
      })
