class MyFirmProfileAUMTRController extends BaseController
  @register 'MyFirmProfileAUMTRController'

  @inject 'FirmDataservice', 'ModalFactory', '$scope', 'Restangular',
          '$stateParams', 'SweetAlert', 'toaster', 'keywordConstants', 'Utils','angularEnabled','$window','$state'
  ...
  getTables: () ->
    @FirmDataservice.getTables(@firm.id).then (response)=>
      @tables = response
