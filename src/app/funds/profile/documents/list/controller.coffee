class FundsDocumentsListController extends BaseController

  @register 'FundsDocumentsListController'
  ...
  initialize: ->
    ...
    @documents = @DocumentListResource.$new({entity_id: @$stateParams.fundId, entity_type: 'Fund', q: @searchText})
    @searchResultTemplateUrl = 'shared/ui-grid-cell-templates/search-result-popover.html'
    @$scope.$parent.FundsDocumentsListController = @
  ...
  getAttachments: ->
    id = @$stateParams.fundId
    @FundDataservice.getAttachments(id).then (response) =>
      totalRecords = response.meta.totalRecords
      @FundDataservice.getAttachments(id, totalRecords).then (response) =>
        response
  ...
  generatePageUrl: (entity)=>
    pageUrl = "app/funds/#{entity.entity_id}/profile/documents/#{entity.id}"
    pageUrl
