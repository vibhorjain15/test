class FundProfileAUMTRController extends BaseController
  @register 'FundProfileAUMTRController'

  @inject 'FundDataservice', '$state', 'ModalFactory', '$scope', 'Restangular',
          '$stateParams', 'SweetAlert', 'toaster', 'Utils', 'keywordConstants','angularEnabled',
          '$window', 
  ...
  getTables: () ->
    @FundDataservice.getShareClassTables(@fund.parentFirm.id, @fund.id).then (response)=>
      @tables = response
