class DvSentToInfoController extends BaseController
  @register 'DvSentToInfoController'
  ...
  getRecipients: ->
    @remaining = 0
    @Restangular.all('entitysubscribers').getList(entity_id: @diligence.id, entity_type: 'Duediligence', firm_id: @diligence.tofirm_id).then (response) =>
  ...
  removeSubscriber:(subscriber) =>
    @saving_contact = true
    @Restangular.all('entitysubscribers', subscriber.id).remove(entity_id: @diligence.id, entity_type: 'Duediligence', user_id: subscriber.user_id,tofirm_id: @diligence.tofirm_id).then ((response) =>
