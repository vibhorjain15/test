angular.module('diligenceVault').factory 'InvestmentsResource', ($timeout, Restangular, GridResourceService, BaseDataService, Utils, GridsDataService, uiGridConstants, statusLabel) ->

  entity_type = Utils.getEntityType()
  entity_sub_type = Utils.getEntitySubType()

  new class InvestmentsResource
    ...
    $new: (options) ->
      ...
      resource.name 'service/dvapi_service/fund_search',
        lastTouchPointDate: ->
      ...
      resource.column('strategyName').showAggregationOptions(false).enableHiding(false).setWidth(grid_widths_map['sm_column_lg']).setTemplate('empty-cell').title entity_sub_type

      resource.column('relationship_status_name').showAggregationOptions(false).enableHiding(false).title('Relationship Status').align('center').setWidth(grid_widths_map['sm_column_sm']).setTemplate('empty-cell').filterable(true, {
