class ManageCustomSearchController extends ModalController

  @register 'ManageCustomSearchController'
  ...
  initialize: ->
    ...
    @custom_criteria_options = @search_filters_response.custom_filters
    @entityType = @Utils.getDisplayEntityType(@search_filters_response.response_type)
  ...
  setSearchCriteriasOptions: (filter, index) =>
    if index < @default_criteria_options.length
      newCriteria = {condition: filter.condition, advance_filter_value: filter.advance_filter_value}
      newCriteria.criteria_obj = @default_criteria_options[index]
      @search_criterias.push(newCriteria)
    else
      indexInCustom = _(@custom_criteria_options).findIndex (criteria)=>
        criteria.filter_key == filter.criteria_obj.filter_key
      if indexInCustom > -1
        newCriteria = {condition: filter.condition, advance_filter_value: filter.advance_filter_value}
        newCriteria.criteria_obj = @custom_criteria_options[indexInCustom]
        @search_criterias.push(newCriteria)
