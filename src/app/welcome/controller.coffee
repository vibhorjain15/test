class WelcomeController extends BaseController
  ...
  @inject '$stateParams', 'Utils', 'LayoutUtils', '$state','BaseDataService'

  initialize: ->
    @isLastOneOpen = false
    @help_mode = @$stateParams.help
    ...
    @faqs = [
      ...
      }
      {
        ...
        questions:[
          ...
          {
            id: 28,
            text: "Can I convert/change the response type if we have associated responses?",
            is_premium: true
            answer: '<p>You cannot convert all the response types when you have associated responses. But you can make the below changes:
                      <ul><li>Single line response type can be converted to paragraph*</li>
                        <li>Dropdown response type can be converted to multiple checkbox*</li>
                        <li>Email response type can be converted into one line or paragraph*</li>
                        <li>Numeric decimal response type  to integer and vice versa is allowed</li>
                        <li>Yes/No response type can be converted to Yes/No with explanation for Yes or No and vice versa is allowed</li>
                        <li>Grids and customize grids/tables response types cannot be converted</li></ul>
                      *=vice versa is not allowed</p>'
          }
          {
            id: 29,
            text: "What can I edit if the question has associated responses?",
            is_premium: true
            answer: '<p> You can edit the question text as required. For checkbox/dropdown type questions you can add/remove, modify the options and change ordering. For grid and custom grid you can modify the options, add/remove the columns and rows, change ordering as needed.  </p>'
          }
          {
            id: 30,
            text: "In what scenarios will I see the latest changes (questions added/deleted/modified) of the template in the project without activating the template?",
            is_premium: true
            answer: '<p> In all the below scenarios you will see the latest changes without activating the template:
                      <ul>
                        <li>When there are no responses available for the questions.</li>
                        <li>When all the external projects of this template version have an “Invited” status.</li>
                        <li>When all the Internal projects of this template version have a “Started” status.</li>
                      </ul>
                    </p>'
          }
        ]
    ...
    @dd_create_chardin_config =
      reveal_menubar: true
      intros: [
        {
          target: '.js-menu-new-action',
          intro: 'Hover & click "Information Request" to create a diligence / survey request',
          visible_elements: '.js-navbar-default'
        }
      ]

    @dd_manage_chardin_config =
      reveal_menubar: true
      intros: [
        {
          target: '.js-menu-diligence',
          intro: 'Hover & click "Projects" to manage your due diligence projects',
          visible_elements: '.js-navbar-default'
        }
      ]

    @template_create_chardin_config =
      reveal_menubar: true
      intros: [
        {
          target: '.js-menu-new-action',
          intro: 'Hover & click "Template" to create a new Template',
          visible_elements: '.js-navbar-default'
        }
      ]

    @template_manage_chardin_config =
      reveal_menubar: true
      intros: [
        {
          target: '.js-menu-diligence',
          intro: 'Hover & click "Templates" to manage your Templates',
          visible_elements: '.js-navbar-default'
        }
      ]

    @products_create_chardin_config =
      reveal_menubar: true
      intros: [
        {
          target: '.js-menu-new-action',
          intro: 'Hover & click "Product" to add a new Product',
          visible_elements: '.js-navbar-default'
        }
      ]

    @compare_chardin_config =
      reveal_menubar: true
      intros: [
        {
          target: '.js-menu-analyze'
          intro: 'Hover & click "Compare" to compare due diligence responses'
          visible_elements: '.js-navbar-default'
        }
      ]
