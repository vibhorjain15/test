class StrategiesDocumentsListController extends BaseController

  @register 'StrategiesDocumentsListController'
  ...
  getAttachments: ->
    id = @$stateParams.strategyId
    @StrategyDataservice.getAttachments(id).then (response) =>
      totalRecords = response.meta.totalRecords
      @StrategyDataservice.getAttachments(id, totalRecords).then (response) =>
        response
  ...
  generatePageUrl: (entity)=>
    pageUrl = "app/strategies/#{entity.entity_id}/profile/documents/#{entity.id}"
    pageUrl
