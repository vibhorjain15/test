class StrategyProfileAUMTRController extends BaseController
  @register 'StrategyProfileAUMTRController'

  @inject 'StrategyDataservice', '$state', 'ModalFactory', '$scope', 'Restangular',
  ...
  getTables: () ->
    @StrategyDataservice.getShareClassTables(@strategy.parentFirm.id, @strategy.id).then (response)=>
      @tables = response
  ...
  backToStrategies: =>
    if @isInvestor
      @$state.go "app.monitor.strategies"
    else
      @$state.go "app.monitor.strategies"
